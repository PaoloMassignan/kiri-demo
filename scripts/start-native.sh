...
#
# Usage:
#   bash scripts/start-native.sh              # OAuth or API key mode
#   bash scripts/start-native.sh claude       # Force Claude Code
#   bash scripts/start-native.sh opencode     # Force OpenCode
#
# Prerequisites:
...
#
# Fallback to Docker: bash scripts/start.sh
...
if [[ -z "$TOOL" ]]; then
    ...
    if [[ -z "$TOOL" ]]; then
        ...
        echo "Install one, or pass it explicitly: bash scripts/start-native.sh opencode" >&2
        kill "$KIRI_PID" 2>/dev/null || true
        rm -f .kiri/.mode
        exit 1
    fi
